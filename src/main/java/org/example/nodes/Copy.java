...
/*
    ...
    such that the pointers in the original list and copied list represent the same list state. None of
    ...
    public Node copyRandomList(Node head) {
        ...
        while(temp != null){
            if(!result.containsKey(temp)){
                result.put(temp, new Node(temp.val));
            }

            if(temp.next != null){
                if(!result.containsKey(temp.next)){
                    result.put(temp.next, new Node(temp.next.val));
                }
                result.get(temp).next = result.get(temp.next);
            }

            if(temp.random != null){
                if(!result.containsKey(temp.random)){
                    result.put(temp.random, new Node(temp.random.val));
                }
                result.get(temp).random = result.get(temp.random);
            }
